...
自动搬运服务 `bili-mover` 稳定运行，**无待办故障**。08-29 修的四处问题都在生产上验证过了。

积压已清空：09-05 一天内 25 条命中、23 条投稿成功，2 条挂在 YouTube 风控偶发失败
（约 8%，重跑即可，无需处理）。

**产出速率是过滤规则决定的，不是故障。** 09-05 18:40 之后曾连续 30 小时零产出，
查下来是频道那两天确实没有既命中 `#newsevery` 又不命中排除词的视频。
排查这类「没更新」问题的正确姿势见 [RUNBOOK §8](./RUNBOOK.md#8-诊断产出为零--最近没更新)，
先看判定分布再怀疑故障——过去两次都是过滤规则，不是服务挂了。
