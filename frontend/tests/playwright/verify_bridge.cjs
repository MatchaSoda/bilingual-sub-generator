...
(async () => {
  ...
  try {
    ...
    await page.fill('input[placeholder*="PASTE"]', 'https://www.youtube.com/watch?v=verify');
    
    console.log('Clicking Engage button...');
    await page.click('button:has-text("Engage")');
    
    console.log('Monitoring telemetry...');
    // 等待跳转到 Telemetry 面板
    await page.waitForTimeout(2000);
    await page.screenshot({ path: path.join(__dirname, 'test_engage_click.png') });
